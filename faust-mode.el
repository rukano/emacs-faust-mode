...
(defconst faust-faust-keywords-statements
...
(defconst faust-faust-keywords-functions
...
(defconst faust-faust-keywords-math
  '("mem" "prefix" "int" "float" "rdtable" "rwtable" "select2" "select3" "ffunction" "fconstant" "fvariable" "attach" "acos" "asin" "atan" "atan2" "cos" "sin" "tan" "exp" "log" "log10" "pow" "sqrt" "abs" "min" "max" "fmod" "remainder" "floor" "ceil" "rint"))

(defconst faust-faust-keywords-ui
  '("button" "checkbox" "vslider" "hslider" "nentry" "vgroup" "hgroup" "tgroup" "vbargraph" "hbargraph"))

;; Lib!

(defconst faust-faust-keywords-lib-analyzer
...
(defconst faust-faust-keywords-lib-basic
...
(defconst faust-faust-keywords-lib-compressor
  '("compressor_mono" "compressor_stereo" "limiter_*"))

(defconst faust-faust-keywords-lib-delay
  '("delay" "fdelay" "sdelay" "fdelaylti" "fdelayltv" "fdelay[n]" "fdelay[n]a"))

(defconst faust-faust-keywords-lib-demo
...
(defconst faust-faust-keywords-lib-envelope
  '("smoothEnvelope" "ar" "asr" "adsr"))

(defconst faust-faust-keywords-lib-filter
...
(defconst faust-faust-keywords-lib-misceffect
...
(defconst faust-faust-keywords-lib-miscoscillator
...
(defconst faust-faust-keywords-lib-noise
  '("noise" "multirandom" "multinoise" "noises" "pink_noise" "pink_noise_vm" "lfnoise" "lfnoise0" "lfnoiseN"))

(defconst faust-faust-keywords-lib-phafla
  '("flanger_mono" "flanger_stereo" "phaser2_mono" "phaser2_stereo"))

(defconst faust-faust-keywords-lib-pm
  '("chain" "input" "output" "terminations" "fullTerminations" "leftTermination" "rightTermination" "waveguide" "idealString"))

(defconst faust-faust-keywords-lib-reverb
  '("jcrev" "satrev" "mono_freeverb" "stereo_freeverb" "fdnrev0" "zita_rev_fdn" "zita_rev1_stereo" "zita_rev1_ambi"))

(defconst faust-faust-keywords-lib-route
  '("cross" "crossnn" "crossn1" "interleave" "butterfly" "hadamard" "recursivize"))

(defconst faust-faust-keywords-lib-signal
  '("bus" "block" "interpolate" "smooth" "smoo" "polySmooth" "bsmooth" "lag_ud" "dot"))

(defconst faust-faust-keywords-lib-spat
  '("panner" "spat" "stereoize"))

(defconst faust-faust-keywords-lib-synth
  '("popFilterPerc" "dubDub" "sawTrombone" "combString" "additiveDrum" "additiveDrum"))

(defconst faust-faust-keywords-lib-vaeffect
  '("moog_vcf" "moog_vcf_2b" "wah4" "autowah" "crybaby" "vocoder"))

(defvar faust-faust-keywords-lib
  (append
   faust-faust-keywords-lib-analyzer
   faust-faust-keywords-lib-basic
   faust-faust-keywords-lib-compressor
   faust-faust-keywords-lib-delay
   faust-faust-keywords-lib-demo
   faust-faust-keywords-lib-envelope
   faust-faust-keywords-lib-filter
   faust-faust-keywords-lib-hoa
   faust-faust-keywords-lib-math
   faust-faust-keywords-lib-misceffect
   faust-faust-keywords-lib-miscoscillator
   faust-faust-keywords-lib-noise
   faust-faust-keywords-lib-phafla
   faust-faust-keywords-lib-pm
   faust-faust-keywords-lib-reverb
   faust-faust-keywords-lib-route
   faust-faust-keywords-lib-signal
   faust-faust-keywords-lib-spat
   faust-faust-keywords-lib-synth
   faust-faust-keywords-lib-vaeffect)
  "All the Faust library function keywords.")

(defvar faust-mode-ac-source
  '((candidates . faust-faust-keywords-lib)))

(defvar faust-regexp-keywords-function (regexp-opt faust-faust-keywords-functions 'words))
(defvar faust-regexp-keywords-statement (regexp-opt faust-faust-keywords-statements 'words))
(defvar faust-regexp-keywords-ui (regexp-opt faust-faust-keywords-ui 'words))
(defvar faust-regexp-keywords-lib (regexp-opt faust-faust-keywords-lib 'words))
